...
import { AFew, No } from "@/components/svg/Stock";
...
const Items: FC<{ isGoods?: boolean }> = ({ isGoods = false }) => {
  ...
  return (
    <div className="m-4 my-8 md:m-8 md:my-16 grid gap-4 gap-y-[calc(1rem/cos(8deg))] grid-cols-3 md:grid-cols-4 skew-y-[4deg]">
      {(isGoods ? goods : souvenirs).map((item, i) => (
        <Frame key={item.id[0]} isGoods={isGoods} item={item} isLarge={i === 0} useSWRType={{ data, error, isLoading }} />
      ))}
    </div>
  );
}

const Frame: FC<{ isGoods?: boolean, item: Item; isLarge?: boolean, useSWRType: UseSWRType }> = ({ isGoods = false, item: { id, size, name, imageCount, price, description, types }, isLarge = false, useSWRType: {data, error, isLoading } }) => {
