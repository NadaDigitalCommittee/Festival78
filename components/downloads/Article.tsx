      ...
      <AccordionPanel>
        {articles?.map((article, i) => (
          ...
          w-full
            ...
            px-12
            text-sm
            group-odd:odd:bg-white/50 group-odd:even:bg-[#FFEFE8]/50 group-even:odd:bg-[#FFEFE8]/50 group-even:even:bg-white/50
            `}
          >
            <span className="mr-4">{article.title}</span>
            <a
              href={article.url}
              className="ml-auto flex h-[30px] min-w-[130px] items-center justify-center gap-2 bg-theme px-2 text-center text-white outline-focus-visible"
              download
            >
              <RiDownload2Fill />
              ダウンロード
